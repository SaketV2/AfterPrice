...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ next?: string; checkout?: string; session_id?: string }> }) {
  ...
  const claimNext = checkoutClaim ? `/app?checkout=claim&session_id=${encodeURIComponent(checkoutClaim.sessionId)}&claim_token=${encodeURIComponent(checkoutClaim.claimToken)}` : undefined
  const next = safeAppPath(params.next ?? claimNext)
  if (data?.claims) redirect(next)
  return <main id="main-content" tabIndex={-1} className={[styles.authFrame, 'flex min-h-[calc(100vh-80px)] items-center justify-center px-5 py-16 sm:px-8'].join(' ')}><AuthForm mode="login" next={next} checkoutClaim={checkoutClaim} /></main>
}
