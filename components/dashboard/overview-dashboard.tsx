...
import { CalendarClock, CircleAlert, CirclePlus, Eye, PiggyBank } from 'lucide-react'
...
function SummaryStrip({ entries }: { entries: ReturnType<typeof buildLedgerEntry>[] }) {
  ...
  return <section aria-label="Monitoring summary" className="grid overflow-hidden rounded-card border border-border bg-surface shadow-soft sm:grid-cols-2 xl:grid-cols-4">{summary.map(({ label, value, helper, icon: Icon, tone }) => <div key={label} className="flex min-h-[104px] items-start gap-3 border-b border-border px-5 py-4 last:border-b-0 sm:border-r sm:last:border-r-0 xl:border-b-0"><Icon aria-hidden className={`mt-1 h-4 w-4 shrink-0 ${tone}`} /><div><p className="text-xs font-semibold text-[hsl(var(--foreground-secondary))]">{label}</p><p className={`mt-2 font-sans text-2xl font-extrabold tracking-[-0.035em] ${tone}`}>{value}</p><p className="mt-1 text-xs leading-5 text-[hsl(var(--foreground-muted))]">{helper}</p></div></div>)}</section>
}
