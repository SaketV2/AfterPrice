  ...
  return <header className={['sticky top-0 z-50 border-b border-transparent', styles.header, scrolled ? styles.headerScrolled : ''].join(' ')}>
    <div className="mx-auto flex min-h-20 w-full max-w-[1280px] items-center justify-between gap-6 px-5 py-3 sm:px-8 lg:px-10">
      ...
      <nav aria-label="Primary navigation" className="hidden items-center gap-1 lg:flex">
        {primaryLinks.map(([label, href]) => { const active = pathname === href; return <Link key={href} href={href} aria-current={active ? 'page' : undefined} className={[styles.headerNavLink, active ? styles.headerNavLinkActive : ''].join(' ')}>{label}</Link> })}
      </nav>
  ...
  return <footer className={styles.footer}>
    <div className={styles.footerInner}>
      <div className={styles.footerGrid}>
        ...
        <FooterColumn title="Account" links={[['Log in', '/login'], ['Sign up', '/signup'], ['View sample', '/demo']]} />
        <FooterColumn title="Read" links={[['Pricing', '/pricing'], ['FAQ', '/faq'], ['Privacy', '/privacy'], ['Terms', '/terms']]} />
      </div>
      <div className={styles.footerBottom}><span>© 2026 AfterPrice · V1</span><span>Potential differences stay labelled until an outcome is recorded.</span></div>
    </div>
  </footer>
}
